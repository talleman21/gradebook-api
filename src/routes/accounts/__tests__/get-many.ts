...
describe("account-get-many", () => {
  ...
  beforeEach(() => {
    ...
    findManyMock = jest.spyOn(prisma.account, "findMany");
    countMock = jest.spyOn(prisma.account, "count").mockResolvedValue(1);
  });
  ...
  it("responds with valid object array", async () => {
    ...
    //when
    transactionMock.mockResolvedValue([1, [rawAccount]]);
    ...
    expect(res.send).toHaveBeenCalledWith([accountDTO]);
  });
